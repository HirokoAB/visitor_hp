	...
	try {
		...
		switch ($arrForm['mode']) {
			...
			case 'upload':
				...
				$arrNotice['error'] = array_merge($arrNotice['error'], $classIssue->check($arrEdit, 'csv'));
				break;
		...
		switch ($arrForm['mode']) {
			...
			// ダウンロード
			case 'download':
				$result = $classIssue->csv_download(['order' => 'issue_id ASC']);
				exit(0);
				break;
			...
			case 'upload':
				try {
					...
					$result = $classIssue->csv_upload($arrEdit);
					$classPdo->commit();	// コミット
				} catch (Exception $e) {
					$classPdo->rollBack();	// ロールバック
					throw new Exception($e->getMessage());
				}
				$classUtility->redirect($arrPath['dirname']. '/csv.php', ['success' => $result. '件の発刊物を上書き保存しました']);
				break;
		...
		// テンプレート設定
		// -----------------------------------------------------
		define('TEMPLATE' , 'admin/issue/csv.tpl'              );	// テンプレートファイル名
		define('SUB_TITLE', '発刊物のCSV管理'                  );	// ページサブタイトル
