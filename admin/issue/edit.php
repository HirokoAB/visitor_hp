	...
	try {
		...
		switch ($arrForm['mode']) {
			...
			case 'delete':
				...
				$arrNotice['error'] = array_merge($arrNotice['error'], $classIssue->check($arrForm, 'delete'));
			...
			case 'save':
				if (false == $classUtility->tokenCheck($arrForm['token'], 'admin')) throw new Exception(400);
				$arrNotice['error'] = array_merge($arrNotice['error'], $classIssue->check($arrEdit, 'save'));
				break;

			// 編集
			case 'edit':
				$arrNotice['error'] = array_merge($arrNotice['error'], $classIssue->check($arrForm, 'edit'));
				if (!$classUtility->isBlank($arrNotice['error'])) $classUtility->redirect($arrPath['dirname']. '/', ['error' => $arrNotice['error']]);
				break;
		...
		switch ($arrForm['mode']) {
			...
			case 'delete':
				try {
					...
					$count = $classIssue->delete($arrForm['issue_id']);
					$classPdo->commit();	// コミット
				} catch (Exception $e) {
					$classPdo->rollBack();	// ロールバック
					throw new Exception($e->getMessage());
				}
				$classUtility->redirect($arrPath['dirname']. '/', ['success' => '発刊物ID：'. $arrForm['issue_id']. ' を削除しました']);
				break;
			...
			case 'save':
				try {
					$classPdo->begin();	// トランザクション開始
					$arrEdit['issue_id'] = $classIssue->save($arrEdit);
					$classPdo->commit();	// コミット
				} catch (Exception $e) {
					$classPdo->rollBack();	// ロールバック
					throw new Exception($e->getMessage());
				}
				$classUtility->redirect($arrPath['dirname']. '/', ['success' => '発刊物ID：'. $arrEdit['issue_id']. ' を保存しました']);
				break;

			// 編集
			case 'edit':
				$arrSearch = [
					'issue_id'     => $arrForm['issue_id'],
					'delete_flag' => 0,
					'limit'       => 1
				];
				$arrColumn = [
				...
				$arrRet = $classIssue->search($arrSearch, $arrColumn);
		...
		// テンプレート設定
		// -----------------------------------------------------
		$sub_title = ($classUtility->isBlank($arrEdit['issue_id'])) ? '発刊物の新規登録': '発刊物ID：'. $arrEdit['issue_id']. ' の編集';
		define('TEMPLATE' , 'admin/issue/edit.tpl'             );	// テンプレートファイル名
